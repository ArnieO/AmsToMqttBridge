...
void loop()
{
    ...
    if (hasMaster)
    {
        // Disable: Red and Green LED on RGB shall be held during lightsleep
        ESP_ERROR_CHECK(gpio_hold_dis((gpio_num_t)config.getLedPinGreen()));
        ESP_ERROR_CHECK(gpio_hold_dis((gpio_num_t)config.getLedPinRed()));
    ...
    {
        hw.ledOn(LED_YELLOW);
        // Enable: Red and Green LED on RGB shall be held during lightsleep
        ESP_ERROR_CHECK(gpio_hold_en((gpio_num_t)config.getLedPinGreen()));
        ESP_ERROR_CHECK(gpio_hold_en((gpio_num_t)config.getLedPinRed()));
    }
    if (config.getApPin() != 0xFF)
    {
        if (digitalRead(config.getApPin()) == LOW)
        {
            ...
            if ((now - buttonTimer > longPressTime) && (longPressActive == false))
            {
                ...
                hasMaster = false;
                debugI("Long press AP button detected");
            }
        }
        //********************************************************************
		else
